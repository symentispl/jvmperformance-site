...
```bash
tpay-pay-by-link$ sam logs -n HelloEfsFunction --stack-name tpay-pay-by-link --tail
```
...
```bash
tpay-pay-by-link$ pip install -r tests/requirements.txt --user
# unit test
tpay-pay-by-link$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
tpay-pay-by-link$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name tpay-pay-by-link
```
